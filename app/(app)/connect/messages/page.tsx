...
const THREADS = [
  { id: 1, avatar: 'M', color: 'var(--blue)', bg: 'var(--blue-mid)', name: 'Marcus T.', preview: 'Want to share your pitch deck first?', time: '2m ago', unread: 2, online: true, kind: 'single', source: 'Marketplace inquiry' },
  { id: 2, avatar: 'A', color: 'var(--green)', bg: 'var(--green-mid)', name: 'Aisha R.', preview: 'I redesigned the logo concept.', time: '1h ago', unread: 0, online: true, kind: 'single', source: 'Profile connection' },
  { id: 3, avatar: '🚀', color: '#fff', bg: 'var(--dark2)', name: 'Brand Sprint Group', preview: 'Let’s submit by Friday night.', time: '3h ago', unread: 0, online: false, kind: 'group', source: 'Group thread' },
]
...
export default function MessagesPage() {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '0', height: 'calc(100vh - 180px)', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '14px', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>

      {/* Thread list */}
      <div style={{ borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '16px', borderBottom: '1px solid var(--border)' }}>
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search messages…" style={{ width: '100%', padding: '8px 12px', borderRadius: '7px', border: '1px solid var(--border)', background: 'var(--surface)', fontSize: '13px', color: 'var(--text)', outline: 'none', fontFamily: 'var(--font-body)' }} />
        </div>
        {filteredThreads.map((t) => (
          <button key={t.id} onClick={() => setActiveThread(t.id)} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: '10px', color: 'var(--text3)' }}>{t.kind === 'group' ? 'Group chat' : 'Direct message'}</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '10px', background: 'var(--card)' }}>
          ...
          <Link href="/profile" style={{ marginLeft: 'auto', textDecoration: 'none', fontSize: '12px', color: 'var(--blue)' }}>View profile</Link>
        </div>
        <div style={{ borderBottom: '1px solid var(--border)', padding: '8px 20px', fontSize: '11px', color: 'var(--text3)' }}>{active.source}</div>
        ...
        <div style={{ padding: '14px 16px', borderTop: '1px solid var(--border)', display: 'flex', gap: '10px' }}>
          ...
          <button onClick={() => {
            ...
            setInput('')
            setThreads((prev) => prev.map((t) => t.id === activeThread ? { ...t, preview: input, time: 'now' } : t))
          }} style={{ padding: '10px 22px', borderRadius: '9px', background: 'linear-gradient(135deg, #16a34a, #22c55e)', border: 'none', color: '#fff', fontSize: '13px', fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-display)' }}>
            Send
          </button>
        </div>
      </div>
